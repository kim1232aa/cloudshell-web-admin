...
- [ ] **步骤 6：手工验证 force-flag 逻辑**

`watchdog.sh` 末尾会自动跑 `check_once`，需要 `gcloud` 在 PATH 上——把
`tests/fixtures` 加进 PATH 用任务7的假 `gcloud` 顶上，避免打真实 Google API；
`PROBE_RETRIES=1` 把探活重试压到最短，避免默认4次重试(每次夹3秒sleep)拖长
验证耗时：

```bash
mkdir -p /tmp/wd-test
PATH="$PWD/tests/fixtures:$PATH" STATE_DIR=/tmp/wd-test TUNNEL_HOST=example.invalid \
  INTERVAL=3600 PROBE_RETRIES=1 timeout 20 bash watchdog.sh --loop &
sleep 3
touch /tmp/wd-test/force-failover-requested
sleep 14
cat /tmp/wd-test/watchdog.log   # 预期看到 "force-failover flag seen" 这行
ls /tmp/wd-test/force-failover-requested   # 预期文件已不存在
```
...
services:
  ...
  web-admin:
    ...
    restart: unless-stopped
    environment:
      ADMIN_PASSWORD_HASH: "${ADMIN_PASSWORD_HASH:?run python3 web_admin/hash_password.py first}"
