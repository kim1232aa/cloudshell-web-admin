...
tests/
  ...
  fixtures/gcloud          # 测试用假 gcloud 脚本，不联网不需要真凭据（新建）
...
RUN pip install --no-cache-dir --break-system-packages pysocks==1.7.1
...
```bash
git add web_admin/gcloud_accounts.py tests/test_gcloud_accounts.py tests/fixtures/gcloud
...
mkdir -p "$STATE_DIR"
export PATH="$PWD/tests/fixtures:$PATH"   # 用 gcloud，不需要真凭据
export ADMIN_PASSWORD_HASH="$(python3 -c "
import sys; sys.path.insert(0,'web_admin'); import auth; print(auth.hash_password('testpass123'))
")"
export SESSION_SECRET=test-secret
...
4. 添加一个代理 → 表格刷新出现新行
5. 点"添加账号"，输入 `acct-demo`，选一个代理，提交 → 弹窗显示 gcloud（若本地
